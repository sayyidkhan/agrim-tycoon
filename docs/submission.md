...
Gemma is the local chief of staff inside the game. It runs on-device through a bundled `llama.cpp` runtime, triaging incidents and stress-testing the Machine City charter as structured recommendations, narratives, and stat changes. Players can act on or challenge its recommendation, so Gemma materially changes the game loop rather than sitting beside it as a chatbot. The desktop build remains playable offline, with an explicit deterministic fallback if the local runtime is unavailable.

Judges should play the Machine City charter scenario and ask Gemma for guardrails. The result visibly shifts civic control, acceleration, and the next decision in the city.
...
| 1:35–2:20 | Machine City stress test | Ask Gemma for launch guardrails; show the structured branch and visible city-stat changes. |
